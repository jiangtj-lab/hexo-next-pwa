...
module.exports = (hexo, opts) => {
  ...
  if (opts.enable) {
    console.register(
      ...
      "Generate service-worker files.",
      {
        options: [
          {
            name: "-g, --generate",
            desc: "Exec hexo generate before workbox build.",
          },
        ],
      },
      (args = {}) => {
    ...
    if (opts.wrapGenerate) {
      const oldGenerate = console.get("generate");
      console.register(
        "generate",
        oldGenerate.desc + "(Override by hexo-next-pwa)",
        oldGenerate.options,
        (args = {}) => {
          oldGenerate
            .bind(hexo)(args)
    ...
    if (opts.addScript) {
      ...
      const compiled = template(
        readFileSync(join(__dirname, "./web-page-script.html"), "utf8"),
      );
